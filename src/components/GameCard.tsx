  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Top badges — mobile: stacked full-width row, desktop: absolute corners */}
        {/* Top-left: GAME + CONTRIBUTION */}
        <div className="absolute top-3 left-3 z-10 flex flex-col gap-1">
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-bold
            bg-primary/20 backdrop-blur-md border border-primary/40 text-primary tracking-wider uppercase self-start">
            <Gamepad2 className="w-2.5 h-2.5 flex-shrink-0" />
            Game
          </span>
          {project.isContribution && (
            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-semibold
              bg-primary/90 text-primary-foreground backdrop-blur-sm self-start">
              <GitFork className="w-2.5 h-2.5 flex-shrink-0" />
              Contribution
            </span>
          )}
        </div>

        {/* Top-right: Platform + Engine — column on mobile to avoid overlap */}
        <div className="absolute top-3 right-3 z-10 flex flex-col items-end gap-1">
          ...
          {project.engine && (
            <span className="px-2 py-0.5 rounded-lg text-[10px] font-semibold bg-background/70 backdrop-blur-md border border-border/50 text-muted-foreground">
              {project.engine}
            </span>
          )}
        </div>
        ...
        <div className="absolute bottom-0 left-0 right-0 p-3 md:p-5 z-10">
          <div className="flex flex-wrap gap-1 mb-2">
            {project.genre?.map((g) => (
              <span key={g}
                className={`px-1.5 py-0.5 rounded-md text-[10px] font-bold border backdrop-blur-sm tracking-wide
          ...
          <div className="flex items-end justify-between gap-2">
            <div className="flex-1 min-w-0">
              <h3 className="text-base md:text-xl font-bold text-foreground leading-tight mb-0.5
                group-hover:text-primary transition-colors duration-300">
                {project.title}
              </h3>
              <p className="text-[11px] md:text-xs text-muted-foreground/80 truncate">{project.description}</p>
            </div>
            ...
            >
              {project.downloadFile && (
                <div className="inline-flex items-center gap-1 px-2 py-1.5 rounded-xl text-[10px] md:text-xs font-bold
                  bg-primary text-primary-foreground shadow-[0_0_16px_-2px_hsl(var(--primary)/0.6)]">
                  View More
                </div>
              )}
              <div className="p-1.5 rounded-xl border border-border/60 bg-background/50 backdrop-blur-sm text-muted-foreground hover:text-foreground transition-colors duration-200">

...
const AnimatedNumber = ({ target, duration = 1000, animKey }: { target: number; duration?: number; animKey: number }) => {
  ...
  return <span className="text-xs md:text-sm font-bold font-mono tabular-nums text-primary">{current}%</span>;
};

const ProficiencyBar = ({ progress, animKey }: { progress: number; animKey: number }) => {
  ...
  return <div className="h-full rounded-full" style={{ width: `${barWidth}%`, background: 'linear-gradient(90deg, hsl(var(--primary)), hsl(var(--primary) / 0.6))' }} />;
};
                ...
                <div>
                  ...
                  <div className="mb-4 md:mb-6">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <AnimatedNumber target={active.progress} duration={1000} animKey={currentAnimKey} />
                    </div>
                    <div className="h-1.5 md:h-2 bg-muted rounded-full overflow-hidden">
                      <ProficiencyBar progress={active.progress} animKey={currentAnimKey} />
                    </div>
                  </div>
                </div>
